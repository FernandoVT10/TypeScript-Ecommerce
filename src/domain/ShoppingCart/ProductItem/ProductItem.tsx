...
function ProductItem({ product }: ProductItemProps) {
	...
	<div className={styles.productItem}>
		...
		{ product.discount > 0 &&
			...
			<s className={styles.oldPrice}>$ { AddSpacesToNumber(product.price * quantity) }</s>
		    </div>
		}

		{ product.discount === 0 && 
		    <div className={styles.price}>
			<span className={styles.currentPrice}>$ { AddSpacesToNumber(product.price * quantity) }</span>
		    </div>
		}
